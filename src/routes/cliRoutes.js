  ...
  cli
    .command(
      ...
      async (argv) => {
        ...
        if (!raw) {
          console.error(
            'Usage: queuectl enqueue \'{"id":"job1","command":"sleep 2"}\' or queuectl enqueue -c "sleep 2" [--id job1]'
          );
          process.exitCode = 1;
          return;
        }
        try {
          ...
          console.log(
            `Enqueued job ${job.id} with command: ${JSON.stringify(
              job.command
            )}`
          );
          console.log(job.id);
    ...
    .command(
      ...
      async (argv) => {
        if ((argv.action || "start") === "start") {
          await workerStartController(argv.__config, argv.count, {
            detach: argv.detach,
          });
        } else {
    ...
    .command(
      ...
      async (argv) => {
        const r = await showJobController(argv.__config, argv.id);
        if (!r.found) {
